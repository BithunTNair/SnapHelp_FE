...
const HomePage = () => {
  ...
  useEffect(() => {
    if (loc.state?.scrollTo) {
      const section = document.getElementById(loc.state.scrollTo);
      if (section) {
        setTimeout(() => {
  ...
  const handleSearch = () => {
    if (!locationInput || !service) {
      alert("Please enter your location and select a service.");
    } else {
      alert(`Finding "${service}" service providers near "${locationInput}"...`);
    }
  };
...
const ServiceCard = ({ icon, title }) => (
  <div className="bg-indigo-50 p-6 rounded-xl shadow-md hover:shadow-xl transition transform hover:-translate-y-1 duration-200 flex flex-col items-center">
...
const FeatureCard = ({ icon, title, desc }) => (
  <div className="bg-white p-6 rounded-xl shadow hover:shadow-md transition transform hover:-translate-y-1 duration-200 text-center flex flex-col items-center">
